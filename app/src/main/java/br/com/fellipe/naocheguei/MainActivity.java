...
import br.com.fellipe.naocheguei.enums.TransportType;
import br.com.fellipe.naocheguei.runnable.LocationTracker;
import br.com.fellipe.naocheguei.service.ContactService;
import br.com.fellipe.naocheguei.service.MapService;
import br.com.fellipe.naocheguei.service.NotificationService;
...
import static br.com.fellipe.naocheguei.enums.TransportType.BICYCLING;
...
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback, LocationListener, GoogleApiClient.OnConnectionFailedListener {
    ...
    public static final long LOCATION_UPDATE_INTERVAL = 60 * 1000;
    ...
    @InjectView(R.id.btn_reduce_tempo)
    Button _reduceTimeButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        verifyBasicInformationsAreLoaded();
    ...
    private void instantiateMap() {
        location = new SimpleLocation(this, false, false, LOCATION_UPDATE_INTERVAL);
    ...
    public void addTime(View view){
        ...
        if(activeDestination == null) {
            ...
            mapService.addActualEstimatedTime(300);
            updateEstimatedTimeText();
            _addTimeButton.setEnabled(true);
        }else{// em viagem
    ...
    public void reduceTime(View view){
        _reduceTimeButton.setEnabled(false);
        if(activeDestination == null) {
            Toast.makeText(getBaseContext(), R.string.msg_error_add_destiny, Toast.LENGTH_SHORT).show();
            _reduceTimeButton.setEnabled(true);
        }else if(activeTransportType == null){
            Toast.makeText(getBaseContext(), R.string.msg_error_select_transport_type, Toast.LENGTH_SHORT).show();
            _reduceTimeButton.setEnabled(true);
        }else if(!isTripActive()){
            mapService.reduceActualEstimatedTime(300);
            updateEstimatedTimeText();
            _reduceTimeButton.setEnabled(true);
        }else{// em viagem
            String extra = "/trip/reduce/"+Session.getInstance().getTrip().getTripId();
